...
public class TrainNumberSearchPanel {
    ...
    private final JLabel groupNumberLabel = new JLabel("    Train number: ");
    ...
    public JComponent makeUI(java.util.List<Object[]> studentList) throws ParseException{
        ...
        for (JComponent r : Arrays.asList(groupNumberLabel, trainNumberField)) {
            searchPanel.add(r);
        }
        ...
        searchButton.addActionListener(actionEvent -> {
            try {
                ...
                if (trainNumber != 0.0) {
                    List<Object[]> foundStudentList = Controller.trainNumberSearch(trainNumber);
                    Frame mainFrame[] = JFrame.getFrames();
                    for (Frame frame : mainFrame) {
                        frame.setVisible(false);
                    }
    ...
    public static void createAndShowGUI(List<Object[]> trainList) throws ParseException {
        ...
        f.getContentPane().add(new TrainNumberSearchPanel().makeUI(trainList));
        f.setSize(1000, 400);
